...
class Pedido(Resource):
    ...
    def put(self, id):
        ...
        if not nuevo_estado:
            return {"message": "Debes especificar el nuevo estado del pedido."}, 400
...
class Pedidos(Resource):
    ...
    def get(self):
        ...
        if status:
            if status not in ESTADOS_VALIDOS:
                return {"message": f"Estado no válido. Opciones: {', '.join(ESTADOS_VALIDOS)}"}, 400
        ...
        if user_id:
            try:
                ...
                return {"message": "user_id debe ser un número entero"}, 400
        ...
        try:
            ...
            return {"message": "min_total y max_total deben ser números válidos"}, 400
        ...
        if sort_by:
            if sort_by not in valid_sort_options:
                return {"message": f"sort_by inválido. Opciones: {', '.join(valid_sort_options)}"}, 400
    ...
    def post(self):
        ...
        if not usuario or usuario.estado != "activo":
            return {"message": "Usuario inválido o suspendido"}, 403
        ...
        if not productos:
            return {"message": "Debes agregar al menos un producto al pedido."}, 400
        ...
        for producto in productos:
            try:
                ...
                return {"message": "El campo 'product_id' debe ser un número entero válido."}, 400

            try:
                cantidad = int(producto.get("quantity", 1))
                if cantidad <= 0:
                    return {"message": f"La cantidad del producto con id {product_id} debe ser mayor que 0."}, 400
            except (ValueError, TypeError):
                return {"message": f"La cantidad del producto con id {product_id} debe ser un número entero válido."}, 400
            ...
            if product.estado != "activo":
                return {"message": f"El producto con id {product_id} está suspendido y no puede ser agregado al pedido."}, 400

            if product.stock is None or product.stock < cantidad:
                return {"message": f"No hay stock suficiente para el producto con id {product_id}."}, 400
